...
function App() {
  ...
  const [selectedModel, setSelectedModel] = useState<'sarima' | 'lstm'>('lstm');
  const [forecastPeriods, setForecastPeriods] = useState<number>(12);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    loadForecastData();
  }, [selectedModel, forecastPeriods]);

  const loadForecastData = async () => {
    setIsLoading(true);
    try {
      const data = await ApiService.getForecast(selectedModel, forecastPeriods);
